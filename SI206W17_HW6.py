...
class Student():
    ...
    def write_programs(self,temp=1):
        self.num_programs=self.num_programs+temp
...
student_tups = iter(zip(names,seniority,programs_written))
...
#student_tups_list = zip(names,seniority,programs_written)
#pdb.set_trace()
...
names_and_productivities = [ (x,y) for (x,y) in zip(names,prod_list)]
...
#print([x.name for x in long_names_list])
...
temp = filter(lambda x:len(x.name)>x.years_UM,programmers)

names_with_not_too_much_seniority = [x.name for x in temp]
...
#pdb.set_trace()
#print(len_check(["sadbfadjskdbfakjsdfbajksdbfkajsbdfjkasdbfkajsbdfajksdfbk","dsfsdf","asdfbahdbfkasdfbdasfasdfadwerwerwdvvvw"]))
#print(len("sdfsdfsdadagdagasdgadagagadagdagadgagagagasg".split()))
...
#        pdb.set_trace()
 #       print(list(len_check(readfiles(["samplehw6_1.txt","samplehw6_2.txt"]))))
